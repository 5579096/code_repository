...
def word_filter_counter(text,filter_words):
    ...
    for word in text.split():
        ...
        for char in word.lower():
            if char.isalpha():
                word_only += char
        word_list.append(word_only)
    output_dict = {}
    
    for y in word_list: #['hello', 'world', 'hello']
        
        if y in filter_words:
            output_dict[y] = output_dict.get(y, 0) + 1
...
def word_filter_counter(text,filter_words):
    ...
    for y in word_list: #['hello', 'world', 'hello']
        ...
        if y in filter_words:
            print(y)
            output_dict[y] = output_dict.get(y, 0) + 1 #adding value do dict

    return output_dict

word_filter_counter("Hello world, hello!", ["hello"]) # Expected output: {'hello': 2}
